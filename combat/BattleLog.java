...
public static String render (List<BattleEvent> batch )
{
    ...
    {switch(e.type)
        {
case CRIT_HIT -> sb.append(e.actor).append(" нанёс КРИТИЧЕСКИЙ урон игроку\n ").append("удар по игроку ").append(e.target).append(" на минус ").append(e.amount).append(" У врага осталось ").append(e.targetHp).append(" хп\n ");
case HIT -> sb.append(e.actor).append( " ударил игрока ").append(e.target).append(" отняв у него ").append(e.amount + " здоровья ").append("\nУ игрока " + e.target + " осталось " + e.targetHp + "ХП! \n");
case HEAL -> sb.append(e.actor).append(" heals ").append(e.amount).append(". HP = ").append(e.targetHp).append("\n");
